...
import datetime
import random
from loss import Loss, compute_dapl_loss, compute_entropy_loss, compute_proto_sep_loss, SupConLoss, ArcConLoss, \
    MultiViewCMILossBundle
import torch.nn.functional as F
from dataloader_GCD import load_data

Dataname = 'Deep_Animal'
label_ratio = 0.7
...
parser.add_argument("--lambda_cat", default=0, type=float, help="Category CMI weight")
parser.add_argument("--lambda_inst", default=0, type=float, help="Instance CMI weight")
...
for ft_epoch in range(1, args.fine_tune_epochs + 1):
    ...
    if all_acc > best_all_acc:
        ...
        torch.save(state, './models/' + args.dataset + '_PGCDbest.pth')
